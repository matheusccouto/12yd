...
- **Drop `b3_round` from the feature schema.** Remove it from `NUMERIC_FEATURES` / `CATEGORICAL_FEATURES` in `src/twelveyards/model.py`, from `FeatureRow` in `src/twelveyards/features.py`, and from the feature build path. The model's only round-specific feature goes away; every prediction becomes round-agnostic. The dashboard's re-score path is then a no-op (passes the same `predictions.jsonl` numbers regardless of match) and can be deleted.
- **Read `preferred_foot` from the existing cached player-page JSON.** Extend `extract_player_metadata` in `src/twelveyards/player_history.py` to scan `pageProps.data.playerInformation[]` for `translationKey="preferred_foot"` and store it on `PlayerMetadata`. No re-scraping needed — the data is already on disk at `data/fotmob_cache/players/{id}/{slug}.json`.
...
12. As a maintainer, I want the 6 missing-tournament shootouts added to the in-scope tournaments in `src/twelveyards/tournaments.py`, so the next pipeline run includes them and the 5.5-year training window is dense, not gappy (currently 2023 has 0 kicks).
