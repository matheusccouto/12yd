...
def test_side_distribution_empty_returns_prior() -> None:  # noqa: D103
    assert side_distribution([], 5) == PRIOR_PROB
    assert side_distribution([], 20) == PRIOR_PROB


def test_side_distribution_single_side() -> None:  # noqa: D103
...
def test_side_distribution_takes_last_n() -> None:  # noqa: D103
    p_l, p_c, p_r = side_distribution(["L"] * 10 + ["R"] * 10, 5)
    assert (p_l, p_c, p_r) == (0.0, 0.0, 1.0)


def test_side_distribution_horizons_nest() -> None:  # noqa: D103
    sides = ["L"] * 10 + ["C"] * 5 + ["R"] * 5
...
def test_side_distribution_history_shorter_than_horizon() -> None:  # noqa: D103
    p_l, p_c, p_r = side_distribution(["L", "C"], 20)
    assert p_l == 0.5  # noqa: PLR2004
    assert p_c == 0.5  # noqa: PLR2004
    assert p_r == 0.0
...
def test_compute_features_no_history_uses_prior() -> None:  # noqa: D103
...
def test_compute_features_no_metadata() -> None:  # noqa: D103
    feats = compute_features([], None, TARGET_DATE)
    assert feats["preferred_foot"] == ""
    assert feats["position"] == ""


def test_compute_features_with_history() -> None:  # noqa: D103
    history = [
    ...
    assert feats["career_penalty_count"] == 5  # noqa: PLR2004
    assert feats["position"] == "midfielder"


def test_compute_features_excludes_kicks_on_or_after_target_date() -> None:  # noqa: D103
    history = [
        _penalty(1, "2024-01-01T00:00:00+00:00", side="L"),
...
def test_compute_features_filters_outside_lookback() -> None:  # noqa: D103
    history = [
...
def test_compute_features_returns_seven_keys() -> None:  # noqa: D103
...
def test_compute_features_position_from_metadata() -> None:  # noqa: D103
    feats = compute_features(
        [], _metadata(player_id=1, position_key="centreback"), TARGET_DATE,
    )
...
def test_categorical_indices() -> None:  # noqa: D103
...
def test_build_training_matrix_empty() -> None:  # noqa: D103
    x, y = build_training_matrix({}, {})
    assert x.shape == (0, 7)
    assert y.shape == (0,)


def test_build_training_matrix_shapes() -> None:  # noqa: D103
    p1_kicks = [
...
def test_build_training_matrix_no_metadata_for_player() -> None:  # noqa: D103
...
def test_build_training_matrix_respects_train_floor() -> None:  # noqa: D103
...
def test_build_training_matrix_multiple_players() -> None:  # noqa: D103
    history = {
...
def test_build_prediction_matrix_empty() -> None:  # noqa: D103
    x = build_prediction_matrix([], {}, {})
    assert x.shape == (0, 7)


def test_build_prediction_matrix_shapes() -> None:  # noqa: D103
    x = build_prediction_matrix(
        [1, 2],
...
def test_build_prediction_matrix_default_target_date() -> None:  # noqa: D103
    x = build_prediction_matrix([1], {}, {})
    assert x.shape == (1, 7)


def test_build_prediction_matrix_no_history_player_gets_prior() -> None:  # noqa: D103
    x = build_prediction_matrix(
        [1],
        {},
        {1: _metadata(player_id=1)},
...
def test_build_prediction_matrix_float_dtype() -> None:  # noqa: D103
    x = build_prediction_matrix([1], {}, {}, target_date=TARGET_DATE)
    assert x.dtype == np.float64


def test_build_prediction_matrix_encodes_categoricals() -> None:  # noqa: D103
    x = build_prediction_matrix(
        [1],
        {},
...
def test_build_prediction_matrix_historical_data_shapes_features() -> None:  # noqa: D103
    history = [
        _penalty(i + 1, f"2024-{i+1:02d}-01T00:00:00+00:00", side="L")
        for i in range(5)
    ]
    x = build_prediction_matrix(
        [1],
